...
The ```wallFollower``` function is responsible for controlling the robot's movement while following the wall. It does so by using a PID controller to adjust the the direction of movement. The function also checks for loop detection by comparing the robot's current position with the knownPoints list. If the robot is detected to be in a loop, it sets the ```driveState``` variable back to ```"WallDetection"```.
The ```rotate_angle``` method is used to rotate the robot by a given angle at a given angular velocity. It uses the PID controller to control the rotation.
